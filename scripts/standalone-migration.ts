...
// usage: DATABASE_URL="postgresql://..." node --experimental-strip-types scripts/standalone-migration.ts
...
function postgresErrorDetails(
  error: unknown,
): { detail?: string; hint?: string } {
  if (typeof error !== "object" || error === null) return {};
  return {
    detail: "detail" in error && typeof error.detail === "string"
      ? error.detail
      : undefined,
    hint: "hint" in error && typeof error.hint === "string"
      ? error.hint
      : undefined,
  };
}
...
async function main(): Promise<void> {
  ...
  if (!DB_URL) {
    ...
    console.error('\nUsage:');
    console.error(
      '  DATABASE_URL="postgresql://user:pass@host:5432/db" node --experimental-strip-types scripts/standalone-migration.ts',
    );
    process.exit(1);
  }
  ...
  try {
    ...
    const { detail, hint } = postgresErrorDetails(error);
    console.error('\nmigration failed');
