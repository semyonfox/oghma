...
function isExpectedDocument(value: unknown): value is ExpectedDocument {
  return (
    isRecord(value) &&
...
function parseExpectedDocuments(value: unknown): ExpectedDocument[] {
  if (!isRecord(value) || !Array.isArray(value.documents) || !value.documents.every(isExpectedDocument)) {
...
if (!urlFile || !outputDir || !logFile) {
  console.error(
    "usage: marker-bench-download.ts URL_FILE OUTPUT_DIR DOWNLOADS_JSONL [EXPECTED_ATTESTATION_JSON]",
  );
  process.exit(2);
}
...
const expected = expectedFile
  ? parseExpectedDocuments(JSON.parse(await readFile(expectedFile, "utf8")) as unknown)
  : null;
