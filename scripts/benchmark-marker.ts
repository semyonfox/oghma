...
type BenchmarkSummary = {
  ...
  estimatedUsdPer1000Pages: number | null;
  avgLatencySec: number;
  p95LatencySec: number;
  failures: MarkerResult[];
  suspiciousOutputs: MarkerResult[];
};
...
function usage(): void {
  console.error(`
Usage:
  MARKER_API_URL=http://host:8000 node --experimental-strip-types scripts/benchmark-marker.ts [options] <pdf-or-dir>...
...
Examples:
  ...
  MARKER_API_TOKEN=secret \\
    node --experimental-strip-types scripts/benchmark-marker.ts --concurrency 1,2,4,8 --repeat 2 ./demo-pdfs
`);
}

function parseArgs(argv: string[]): ParsedOptions {
  const opts: Omit<ParsedOptions, "markerUrl"> & { markerUrl: string | undefined } = {
  ...
  const markerUrl = opts.markerUrl;
  if (!markerUrl) {
  ...
  return { ...opts, markerUrl: markerUrl.replace(/\/+$/, "") };
}

async function collectFiles(inputPaths: readonly string[]): Promise<string[]> {
  const files: string[] = [];
...
async function countPages(filePath: string, buffer: Buffer): Promise<number> {
...
function effectivePages(totalPages: number, pageRange: string): number {
...
async function loadInputs(files: readonly string[]): Promise<Input[]> {
  return Promise.all(
...
async function runPool<InputValue, Result>(
  tasks: readonly InputValue[],
  concurrency: number,
  worker: (task: InputValue) => Promise<Result>,
): Promise<Result[]> {
...
function printTable(summaries: readonly BenchmarkSummary[]): void {
  console.log(
    [
      "conc",
      "ok/req",
...
function printableInputs(inputs: readonly Input[]): Omit<Input, "buffer">[] {
  ...
  if (opts.json) {
    ...
    for (const summary of summaries) {
      for (const failure of summary.failures) {
        console.error(
          `failure concurrency=${summary.concurrency} file=${failure.file} status=${failure.status} error=${String(failure.error)}`,
        );
      }
