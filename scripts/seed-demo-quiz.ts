...
type QuizQuestionType = "mcq" | "fill_in" | "true_false";
...
const DEMO_USER_ID = "549afd7a-01fd-4e1e-8637-29452401c2f2";

// canvas course IDs from actual enrollment
const CT216: CanvasCourse = { courseId: 44269, name: "CT216 Software Engineering I" };
const CT213: CanvasCourse = { courseId: 44267, name: "CT213 Computer Systems & Organization" };
...
async function seed(): Promise<void> {
  ...
  // verify user exists
  const [user] = await sql<UserRow[]>`SELECT user_id, email FROM app.login WHERE user_id = ${DEMO_USER_ID}::uuid`;
  if (!user) {
  ...
  for (const courseData of COURSE_DATA) {
    ...
    let folderId: string;
    const [existingFolder] = await sql<FolderRow[]>`
  ...
  const courses = await sql<CourseDashboardRow[]>`
    SELECT
      n.canvas_course_id,
