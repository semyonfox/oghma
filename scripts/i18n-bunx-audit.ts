...
type ParserResult = {
  ...
  nonLiteralWarnings: string[];
  hadParserError: boolean;
};
...
function parseRecord(value: unknown, source: string): Record<string, unknown> {
  if (!isRecord(value)) {
...
function walkFilesSummary(lines: readonly string[], limit = SAMPLE_LIMIT): string {
  return lines.slice(0, limit).join("\n");
}

function printList(title: string, values: readonly string[], total: number, limit = SAMPLE_LIMIT): void {
...
function expandForPluralAndContext(keys: readonly string[]): string[] {
...
function runParser(): ParserResult {
...
`;
  ...
  return {
    extracted: parseRecord(JSON.parse(raw) as unknown, "parser output"),
    nonLiteralWarnings,
...
): { missingInBase: string[]; unusedInCode: string[] } {
...
function main(): void {
  const baseLocale = parseRecord(
    JSON.parse(readFileSync(BASE_LOCALE, "utf8")) as unknown,
    BASE_LOCALE,
  );
  ...
  const hasWarning =
    nonLiteralWarnings.length > 0 || missingInBase.length > 0 || unusedInCode.length > 0;
  ...
  if (nonLiteralWarnings.length > 0) {
    ...
    printList(
      "nonLiteral",
      nonLiteralWarnings.map((line) => line.trim()),
      nonLiteralWarnings.length,
      24,
    );
  }
  ...
  if (hadParserError && process.env.I18N_PARSER_TOLERATE_ERRORS !== "0") {
    console.error(
      "[i18n:bunx] parser exited non-zero but output was generated. Ignoring due known Bun runtime issue.",
    );
  }
