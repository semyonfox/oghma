...
function summarize(concurrency, started, ended, results, minCharsPerPage) {
...
async function main() {
  ...
  for (const concurrency of opts.concurrency) {
    ...
    summaries.push(summarize(concurrency, started, ended, results, opts.minCharsPerPage));
  }

  if (opts.json) {
    console.log(
      JSON.stringify(
        { markerUrl: opts.markerUrl, pageRange: opts.pageRange || null, files: printableInputs(inputs), summaries },
        null,
        2,
      ),
    );
