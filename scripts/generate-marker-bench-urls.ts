...
type SelectionStrategy = "largest" | "balanced" | "stratified-random";
...
type ParsedOptions = {
  ...
  fromSelection: string | undefined;
  seed: string;
  minBytes: number;
  maxBytes: number;
  json: boolean;
...
type SignedStorageObject = StorageObject & { url: string };
...
function isStorageObject(value: unknown): value is StorageObject {
  if (!isRecord(value) || typeof value.key !== "string" || typeof value.size !== "number") {
    return false;
  }
...
function parseSavedSelection(value: unknown): SavedSelection {
  if (!isRecord(value) || typeof value.bucket !== "string" || !Array.isArray(value.files)) {
    throw new Error("selection file has an invalid shape");
  }
  if (!value.files.every(isStorageObject)) {
    throw new Error("selection file contains an invalid object");
  }
  return { bucket: value.bucket, files: value.files };
}

function isSelectionStrategy(value: string): value is SelectionStrategy {
  return value === "largest" || value === "balanced" || value === "stratified-random";
}

function usage(): void {
  console.error(`
Usage:
  node --experimental-strip-types scripts/generate-marker-bench-urls.ts [options]
...
function parseArgs(argv: string[]): ParsedOptions {
  const opts: Omit<ParsedOptions, "strategy"> & { strategy: string } = {
...
async function listPdfObjects(
  client: S3Client,
  bucket: string,
  prefix: string,
  opts: ParsedOptions,
): Promise<StorageObject[]> {
  const keys: StorageObject[] = [];
...
async function main() {
  ...
  if (opts.fromSelection) {
    const saved = parseSavedSelection(JSON.parse(await readFile(opts.fromSelection, "utf8")) as unknown);
  ...
  const signed: SignedStorageObject[] = [];
