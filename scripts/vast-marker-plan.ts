...
type ApplicationPlan = {
  ...
  dispatch_consumer_enabled: boolean;
  provider: string;
};
...
function requiredString(record: Record<string, unknown>, path: string): string {
  const value = record[fieldName(path)];
  if (typeof value !== "string") fail(`${path} must be a string`);
  return value;
}

function requiredNumber(record: Record<string, unknown>, path: string): number {
  const value = record[fieldName(path)];
...
function requiredBoolean(record: Record<string, unknown>, path: string): boolean {
  const value = record[fieldName(path)];
  if (typeof value !== "boolean") fail(`${path} must be a boolean`);
  return value;
}

function requiredEnvironment(value: unknown): Environment {
  const source = requiredRecord(value, "template.environment");
  const environment: Environment = {};

  for (const [name, entry] of Object.entries(source)) {
    if (typeof entry !== "string") fail(`template.environment.${name} must be a string`);
...
function environmentValue(environment: Environment, name: string): string {
  ...
  if (!value) fail(`template.environment.${name} is required`);
  return value;
}

function loadPlan(planPath: string): VastMarkerPlan {
  const parsed: unknown = JSON.parse(fs.readFileSync(planPath, "utf8"));
  const source = requiredRecord(parsed, "plan");
  const template = requiredRecord(source.template, "template");
  const endpoint = requiredRecord(source.endpoint, "endpoint");
  const workergroup = requiredRecord(source.workergroup, "workergroup");
  const application = requiredRecord(source.application, "application");

  return {
    schemaVersion: requiredNumber(source, "schemaVersion"),
    template: {
      name: requiredString(template, "template.name"),
      image: requiredString(template, "template.image"),
      launch_mode: requiredString(template, "template.launch_mode"),
      disk_space_gb: requiredNumber(template, "template.disk_space_gb"),
      environment: requiredEnvironment(template.environment),
    },
    endpoint: {
      endpoint_name: requiredString(endpoint, "endpoint.endpoint_name"),
      min_load: requiredNumber(endpoint, "endpoint.min_load"),
      min_cold_load: requiredNumber(endpoint, "endpoint.min_cold_load"),
      min_workers: requiredNumber(endpoint, "endpoint.min_workers"),
      target_util: requiredNumber(endpoint, "endpoint.target_util"),
      cold_mult: requiredNumber(endpoint, "endpoint.cold_mult"),
      cold_workers: requiredNumber(endpoint, "endpoint.cold_workers"),
      max_workers: requiredNumber(endpoint, "endpoint.max_workers"),
      max_queue_time: requiredNumber(endpoint, "endpoint.max_queue_time"),
      target_queue_time: requiredNumber(endpoint, "endpoint.target_queue_time"),
      inactivity_timeout: requiredNumber(endpoint, "endpoint.inactivity_timeout"),
    },
    workergroup: {
      endpoint_name: requiredString(workergroup, "workergroup.endpoint_name"),
      template_hash: requiredString(workergroup, "workergroup.template_hash"),
      test_workers: requiredNumber(workergroup, "workergroup.test_workers"),
      cold_workers: requiredNumber(workergroup, "workergroup.cold_workers"),
      gpu_ram: requiredNumber(workergroup, "workergroup.gpu_ram"),
      search_params: requiredString(workergroup, "workergroup.search_params"),
    },
    application: {
      provider: requiredString(application, "application.provider"),
      dispatch_consumer_enabled: requiredBoolean(
        application,
        "application.dispatch_consumer_enabled",
      ),
      dispatch_concurrency: requiredNumber(application, "application.dispatch_concurrency"),
      dispatch_attempts: requiredNumber(application, "application.dispatch_attempts"),
    },
  };
}

function requireEqual(object: object, field: string, expected: unknown): void {
  if (Reflect.get(object, field) !== expected) {
    fail(`${field} must be ${JSON.stringify(expected)}`);
  }
}

function readPortableProfile(profilePath: string): Environment {
  ...
  for (const line of fs.readFileSync(profilePath, "utf8").split(/\r?\n/)) {
    ...
    if (separator <= 0) fail(`invalid portable profile line: ${trimmed}`);
    profile[trimmed.slice(0, separator)] = trimmed.slice(separator + 1);
  }
...
function shellQuote(value: string | number): string {
  ...
  return apostrophe + String(value).replaceAll(apostrophe, escapedApostrophe) + apostrophe;
}

const repositoryRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const planPath = path.join(repositoryRoot, "infra", "vast-marker", "serverless-plan.json");
...
const portableProfile = readPortableProfile(portableProfilePath);
...
const requiredSearchParams: ReadonlyArray<readonly [string, string]> = [
...
const searchParamTokens = new Set(plan.workergroup.search_params.trim().split(/\s+/));
for (const [parameter, message] of requiredSearchParams) {
  if (!searchParamTokens.has(parameter)) fail(message);
}

requireEqual(plan.endpoint, "inactivity_timeout", 300);
if (plan.endpoint.target_util <= 0 || plan.endpoint.target_util > 1) {
...
const admissionConcurrency = Number(
  environmentValue(plan.template.environment, "MARKER_ADMISSION_CONCURRENCY"),
);
if (!Number.isFinite(admissionConcurrency) || admissionConcurrency < 1) {
  fail("MARKER_ADMISSION_CONCURRENCY must be a positive number");
}
if (plan.application.dispatch_concurrency > admissionConcurrency) {
  fail("application dispatch concurrency must not exceed worker admission");
}

const suppliedHash = flagValue("--template-hash");
if (suppliedHash && (suppliedHash.includes("<") || suppliedHash.includes(">") || /\s/.test(suppliedHash))) {
...
if (
  suppliedObjectHost &&
  (suppliedObjectHost.includes("://") || suppliedObjectHost.includes("/") || /\s/.test(suppliedObjectHost))
) {
...
const templateEnvironment = {
  ...
  MARKER_ALLOWED_OBJECT_HOSTS:
    suppliedObjectHost ??
    environmentValue(plan.template.environment, "MARKER_ALLOWED_OBJECT_HOSTS"),
};
...
if (
  !suppliedHash &&
  (image.startsWith("<") || templateEnvironment.MARKER_ALLOWED_OBJECT_HOSTS.startsWith("<"))
) {
  console.log(
