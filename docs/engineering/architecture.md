...
OghmaNotes is a Next.js App Router application with a separate long-running Node worker. The web application owns the UI, HTTP APIs, authentication, chat streaming, and job submission. The worker owns Canvas, upload, and vault background jobs.
...
flowchart LR
    ...
    App --> AI[LLM, embedding, and rerank providers]
...
| Redis | BullMQ transport plus cache and rate-limiting support |
...
[`src/lib/queue.ts`](../../src/lib/queue.ts) is the queue-provider boundary for the `canvas-import` and `extract-retry` lanes.
...
The worker handles Canvas discovery and files, direct extraction, extraction retries, Marker completion callbacks, and vault import/export. See [Import pipeline](import-pipeline.md) for the processing contract and tuning boundaries.
...
Chat supports streaming and non-streaming responses. Sessions and messages are relational data. Assistant messages retain canonical plain `content` and optional structured `parts` for durable tool and error UI.
