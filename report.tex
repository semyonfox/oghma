...
\screenshot{github-issues}{GitHub Issues --- Feature tracking with labels for enhancement, security, infrastructure, and bug categories}
...
This project used AI coding assistants throughout development, primarily Claude Code (Anthropic) with some use of OpenCode and Codex. AI produced code across many parts of the project and helped with debugging --- suggesting fixes, explaining error output, and helping navigate unfamiliar APIs. This section is an honest account of how that collaboration worked in practice and what it actually meant for our development.

\subsection{Scope of AI Involvement}

AI was used as an implementation aid throughout the project. Boilerplate-heavy work --- API route scaffolding, database migrations, i18n translation files, repetitive UI patterns --- was a common target. Beyond code generation, AI assisted in debugging sessions: interpreting AWS error messages, identifying misconfigured IAM policies, and tracing SSR prerendering failures through Next.js build output.

\subsection{What That Required From Developers}

Using AI as a coding aid creates its own problems. Code suggestions still had to be read, understood, and verified before committing --- and in practice they frequently required correction. More significantly, AI tends to solve the immediate problem without considering the wider codebase, which accumulated technical debt that developers had to consciously identify and pay down. Integration bugs, state management inconsistencies, and misaligned abstractions were common byproducts that required hands-on diagnosis rather than further AI prompting.

No architectural decision was delegated. Every pivot described in Section~\ref{sec:pivots} --- from VPC networking failures to embedding model right-sizing to the pnpm-to-npm switch --- was diagnosed by developers through production logs, hardware profiling, and documentation. AI cannot observe a cloud environment, hold context across weeks of iteration, or weigh trade-offs against deployment constraints and budget. Those judgments remained entirely human.

\subsection{Learning Through Ownership}

The most important consequence of this workflow was that developers could not treat AI output as a black box. Understanding code well enough to review it, debug it when it failed in production, and refactor it when it accumulated debt forced genuine engagement with every layer of the stack.

AWS is the clearest example. At the start of the project, none of the team had meaningful AWS experience. AI could suggest IAM policy JSON and Amplify configuration stubs, but it could not explain \textit{why} a VPC subnet needed to be in the same availability zone as an ECS task, or why ElastiCache required a VPC peering change that then broke RDS connectivity. Working through those failures --- reading CloudWatch logs, consulting AWS documentation, and iterating on broken infrastructure --- produced a working mental model that survived into production. By the end of the project, AWS infrastructure felt navigable rather than opaque. That arc from zero familiarity to confident operation would not have happened if AI had simply produced correct configuration on the first attempt.

\subsection{Reflection}

AI-assisted development shifts the bottleneck from ``can I implement this?'' to ``do I understand what was just implemented?'' The discipline required is not prompt engineering --- it is critical review, technical debt management, and the willingness to dig into failures that AI cannot reach. Used that way, it accelerates execution without replacing the understanding that makes a system maintainable. The learning came from the breakage, not the generation.
