...
What sets it apart: (1)~RAG-powered search over notes and PDFs using semantic retrieval rather than keyword matching; (2)~direct Canvas LMS integration with one-click course import handled by a self-scaling worker; and (3)~a full AWS production stack built to real-world cloud engineering standards. The app also supports 12 languages through a custom internationalisation system.
...
OghmaNotes supports email/password registration (with SES-based email verification and password reset) and OAuth~2.0 via Google and GitHub using Auth.js. Accounts are deduplicated by email, and rate limiting is applied to all auth endpoints. The full authentication flow is detailed in Section~\ref{sec:flow-auth}.
...
\textbf{Lesson:} External API dependencies introduce constraints beyond cost. Rate limits can silently degrade user experience. Self-hosting critical pipeline components gives control over throughput at the cost of operational responsibility.
...
\textbf{Lesson:} Model selection needs to account for the full inference environment, not just benchmark quality. A smaller model running entirely on GPU will consistently outperform a larger one partially offloaded to CPU. Right-sizing to the available hardware eliminated the erratic latency and improved embedding throughput by roughly 10$\times$.
...
Task tracking ran through GitHub Issues and Pull Requests. The repository accumulated 117 issues (96 closed, 21 open) and 54 pull requests over the project lifetime. Issues were categorised by feature area (authentication, editor, Canvas integration, RAG, infrastructure) and assigned to team members. Pull requests required code review before merging to \tech{main}, with notable PRs including security hardening (PR~\#155), the quiz system (PR~\#135), and a batch sprint (PR~\#133 resolving 37 issues).

Open issues at project close include centralising RAG pipeline constants, converting worker files to TypeScript, adding CloudWatch metrics for the embedding pipeline, and addressing remaining security advisories. An honest backlog of known technical debt.
...
Most of the work happened in February and March 2026, with the RAG pipeline, Canvas import worker, and quiz system all landing within a six-week window. Earlier months were spent on infrastructure setup and the bigger architectural decisions; the final weeks were mostly hardening, docs, and compliance. Dependabot kept a trickle of dependency updates coming throughout.
...
\textbf{Analytics Dashboard:} A dashboard showing study time trends, quiz score history, and weak topic identification would give students more insight into their learning patterns. The current quiz dashboard has a summary widget (streak, mastery percentage, cards due today); full time-series analytics are a planned next step.
...
This project used AI coding assistants throughout development, primarily Claude Code (Anthropic) with some use of OpenCode and Codex. This section describes how the team worked with these tools.

\subsection{How AI Was Used}

AI was used as an implementation accelerator. Developers defined all architectural decisions, feature priorities, and design constraints; AI tools generated code against those specs, which was then reviewed, tested, and adjusted before committing. Common uses included boilerplate for repetitive patterns (API routes, database migrations, i18n translation files), working through AWS configuration issues, and UI refinement.

\subsection{What AI Did Not Do}

AI did not make architectural decisions. Every pivot in Section~\ref{sec:pivots}, from VPC networking failures to embedding model right-sizing, was diagnosed by the developers through production logs, hardware profiling, and AWS documentation. AI generated code; humans debugged it, fixed what broke in production, and made the judgment calls that shaped the system. Diagnosing failures AI couldn't see (cloud networking, GPU memory splits, rate-limit edge cases) turned out to be the most valuable learning experience of the project.

\subsection{Reflection}

AI-assisted development shifts the bottleneck from ``can I implement this?'' to ``should I implement this now?'' Shipping features quickly made scope management the hardest discipline. AI is a force multiplier for execution, but architecture, debugging, and constraint navigation are still fundamentally human work.
