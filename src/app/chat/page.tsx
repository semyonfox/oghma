                    ...
                    {conversations.map((conv) => (
                        ...
                        >
                            <div className="flex items-start justify-between gap-1">
                                <p className="font-medium truncate text-[13px] flex-1">{conv.title}</p>
                                <span
                                    role="button"
                                    tabIndex={0}
                                    onClick={(e) => { e.stopPropagation(); void deleteConversation(conv.id); }}
                                    onKeyDown={(e) => { if (e.key === 'Enter') { e.stopPropagation(); void deleteConversation(conv.id); } }}
                ...
                <header className="flex-shrink-0 flex items-center justify-between px-6 py-3 border-b border-white/8 bg-gray-900">
                    <div className="flex items-center gap-3 min-w-0">
                        <h1 className="text-sm font-medium text-gray-200 truncate">
                            {activeConv?.title ?? (contextPrefix ? `Chat about ${contextPrefix}` : 'New conversation')}
                        </h1>
                        {(activeConv?.noteId || paramNoteId) && (
                            ...
                            >
                                ...
                                <span className="truncate max-w-[150px]">{activeConv?.noteTitle ?? paramNoteTitle}</span>
                            </a>
                        )}
                        {paramFolderId && !activeConv?.noteId && (
                            <span className="flex items-center gap-1 px-2 py-0.5 rounded-full border border-indigo-500/20 bg-indigo-600/10 text-[11px] text-indigo-400">
                                ...
                                <span className="truncate max-w-[150px]">{paramFolderTitle}</span>
