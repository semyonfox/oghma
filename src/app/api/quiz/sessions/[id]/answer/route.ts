...
import { cardFromDB, reviewCard, cardToDB, getNextIntervals } from '@/lib/quiz/fsrs';
...
export const POST = withErrorHandler(async (
    ...
    const { cardId, rating, userAnswer, wasCorrect, responseTimeMs, nextCardId } = body;

    if (!cardId || !rating) return tracedError('cardId and rating are required', 400);
    ...
    const fatigueWarning = answered > 4 && (wrongCount / answered) > SESSION_DEFAULTS.fatigueThreshold;
