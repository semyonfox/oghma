...
export const POST = withErrorHandler(async (request: NextRequest) => {
  ...
  // ── Streaming response — return immediately, do heavy work inside ────────
  if (stream) {
    ...
    return new NextResponse(
      new ReadableStream({
        start(controller) {
          void (async () => {
            try {
              // send SSE comment as first byte so CloudFront keeps the connection open
              controller.enqueue(encoder.encode(": connected\n\n"));

              // ── scope normalization (sync, fast) ──────────────────────────
              ...
              const explicitScopedFolders =
                normalizeScopeItems(selectedFolders);
              ...
              const scopedInputNoteIds = effectiveScope.notes.map(
                (item) => item.id,
              );
              const scopedInputFolderIds = effectiveScope.folders.map(
                (item) => item.id,
              );
              ...
              // ── RAG pipeline (Cohere API — slowest part) ──────────────────
              const {
                searchResults,
                semanticMatches,
                embeddingAvailable,
              ...
              // ── parent folder hint + tool instruction ─────────────────────
              ...
              if (scopedInputNoteIds.length > 0) {
                ...
                if ((parentRows as any[]).length > 0) {
                  const hints = (
                    parentRows as { note_id: string; title: string }[]
                  )
              ...
              const makeMDNote = tool({
                ...
                execute: async ({ text, parentID, title }) => {
                  ...
                  if (parentID) {
                    ...
                    if (!parentRows[0])
                      throw new Error(
                        "parentID does not exist for this user",
                      );
              ...
              const findFolder = tool({
                ...
                execute: async ({ query }) => {
                  ...
                  return {
                    folders: (
                      rows as { note_id: string; title: string }[]
                    ).map((r) => ({
                      id: r.note_id,
                      title: r.title,
                    })),
              ...
              const readNote = tool({
                ...
                execute: async ({ noteId: readNoteId }) => {
                  ...
                  return {
                    noteId: readNoteId,
                    title: row.title,
                    content: (row.content || row.extracted_text || "").slice(
                      0,
                      20_000,
                    ),
                  };
  ...
  // ── Non-streaming path (unchanged — rarely used) ────────────────────────────
