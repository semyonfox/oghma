...
import { cleanCourseName, stripHtmlToText } from "./content-formatting";
import {
  normalizeCanvasCourseSelection,
  type CanvasCourseSelection,
} from "./id";
import type { CanvasRecord } from "./client";

interface RawExportClient {
...
interface RawExportDownloadClient {
...
interface DownloadableFile extends CanvasRecord {
  url: string;
}
...
type QueryValue = string | number | boolean | readonly unknown[] | null | undefined;
...
function sanitizeZipPart(value: unknown, fallback: string) {
...
function entryName(file: CanvasRecord) {
...
function pathWithQuery(path: string, params: Record<string, QueryValue> = {}) {
...
function addJsonEntry(textEntries: TextEntry[], seenPaths: Set<string>, path: string, value: unknown) {
  addTextEntry(textEntries, seenPaths, path, jsonText(value));
}

function addMarkdownEntry(
  textEntries: TextEntry[],
  seenPaths: Set<string>,
  path: string,
  title: unknown,
...
function addJsonEntryIfPresent(textEntries: TextEntry[], seenPaths: Set<string>, path: string, value: unknown) {
  if (value !== null && value !== undefined) {
    addJsonEntry(textEntries, seenPaths, path, value);
  }
}

async function getJson(client: RawExportClient, path: string, label: string, skipped: string[]) {
...
async function getPaginatedJson(client: RawExportClient, path: string, label: string, skipped: string[]) {
...
async function collectFileById(
...
function recordArray(value: unknown): CanvasRecord[] {
...
async function collectSubmissionAttachments(
  downloads: DownloadEntry[],
  ...
  for (const attachment of recordArray(submission.attachments)) {
    collectFile(
      downloads,
      attachment,
  ...
  for (const history of recordArray(submission.submission_history)) {
...
function collectDownloadableAttachments(
  value: unknown,
  basePath: string,
  downloads: DownloadEntry[],
  state: RawExportState,
...
async function collectGenericPaginated(
  client: RawExportClient,
  path: string,
  archivePath: string,
  archive: CanvasRawExportArchive,
...
async function collectGenericObject(
  client: RawExportClient,
  path: string,
  archivePath: string,
  archive: CanvasRawExportArchive,
...
async function collectGroupArchive(
  client: RawExportClient,
  group: CanvasRecord,
  archive: CanvasRawExportArchive,
  state: RawExportState,
...
async function collectConversationArchive(
  client: RawExportClient,
  archive: CanvasRawExportArchive,
  state: RawExportState,
) {
  ...
  const conversationsById = new Map<string, CanvasRecord>();
...
async function collectAccountArchive(
  client: RawExportClient,
  archive: CanvasRawExportArchive,
  state: RawExportState,
) {
...
async function collectCourseArchive(
  client: RawExportClient,
  ...
  const courseSummary: CourseSummary = { id: course.id, title, sources: {} };
  ...
  for (const pageSummary of pages) {
    ...
    const page = await getJson(
      client,
      `/courses/${courseId}/pages/${encodeURIComponent(String(pageSummary.url ?? ""))}`,
    ...
    const revisions = await getPaginatedJson(
      client,
      `/courses/${courseId}/pages/${encodeURIComponent(String(pageSummary.url ?? ""))}/revisions`,
  ...
  for (const announcementSummary of announcements) {
    ...
    if (!isRecord(announcement)) continue;
    addJsonEntry(
      archive.textEntries,
      state.seenPaths,
      `${coursePath}/announcements/${name}.json`,
