...
interface ImportableCanvasFile extends CanvasFile {
  display_name: string;
  filename: string;
  content_type: string;
  url: string;
...
type FileStorage = Pick<StoreS3, "hasObject" | "putObject">;

interface FileImportOptions {
  userId: string;
  courseId: string;
  moduleId: CanvasId | null;
  parentFolderId: string | null;
  client: Pick<CanvasClient, "baseUrl" | "downloadFile">;
  storage: FileStorage;
  s3Prefix: string;
  jobId?: string | null;
...
interface DirectExtractionMessage {
  noteId: string;
  userId: string;
  s3Key: string;
  mimeType: string;
...
interface MarkerContinuationMessage {
  markerJobId?: unknown;
}

interface CanvasImportRecord {
  id: string;
  job_id: string;
  user_id: string;
  status: string;
  ...
  canvas_file_id: CanvasId;
  canvas_module_id?: string | number | null;
  parent_folder_id: string | null;
  s3_prefix: string;
}

interface CanvasImportJoinedRow extends CanvasImportRecord {
  canvas_token: string | null;
  canvas_domain: string | null;
}
...
function assertImportableCanvasFile(file: CanvasFile): asserts file is ImportableCanvasFile {
  if (
    ...
    throw new Error("Canvas file metadata is missing a name, MIME type, or download URL");
  }
}
...
async function reuseImportedPdfCache(
  cache: ImportedFileCacheRow,
  file: ImportableCanvasFile,
...
async function _runFileImport(
  ...
  assertImportableCanvasFile(file);
  const {
    userId,
    courseId,
    moduleId,
...
export async function downloadAndStoreFile(file: CanvasFile, opts: FileImportOptions) {
  ...
  try {
    ...
    try {
      await sql`
        UPDATE app.canvas_imports
        SET status = 'error', error_message = ${errorMessage(error)}, updated_at = NOW()
        WHERE id = ${importRecordId}::uuid
          AND (${opts.jobId ?? null}::uuid IS NULL OR job_id = ${opts.jobId ?? null}::uuid)
...
export async function checkAndCompleteJob(jobId: string, userId: string) {
...
// ── Per-file queue job handler ──────────────────────────────────────────────

function isPermanentCanvasImportError(error: unknown) {
...
export async function processCanvasFile({
  ...
  try {
    ...
    // Idempotency is required because queue delivery is at least once.
    if (
      [
        "complete",
        "forbidden",
...
export async function processExtractionRetry(msg: ExtractionRetryMessage) {
  const {
    noteId,
    userId,
    s3Key,
  ...
  const [existingImport] = await sql<ExistingExtractionImportRow[]>`
    ...
    LIMIT 1
  `;
  let importRow: ExtractionImportRow | null = null;
  if (existingImport) {
    if (
      !importRecordId ||
      !jobId ||
 ...
 */
export async function recoverPendingExtractionRetries(limit: number = 50) {
...
function markerErrorMessage(error: unknown) {
  const message = errorMessage(error);
  return message.replace(/https?:\/\/\S+/gi, "[redacted-url]").slice(0, 1_000);
}

async function finalizeMarkerFailure(
  markerJob: MarkerJobRow,
  status: string,
  message: string,
) {
  let finalized = false;
...
function startMarkerCompletionHeartbeat(markerJob: MarkerJobRow) {
...
export async function processMarkerComplete(msg: MarkerContinuationMessage) {
  const markerJobId = msg?.markerJobId;
  if (typeof markerJobId !== "string") {
    throw new Error("marker-complete is missing markerJobId");
  }

  const maxAttempts = getMarkerCompletionAttemptLimit();
  const [markerJob] = await sql<MarkerJobRow[]>`
    UPDATE app.marker_jobs
  ...
  try {
    ...
    if (
      typeof resultMeta.contentLength === "number" &&
