 ...
 *
 * Fetches courses, modules, assignments, and files from Canvas and inserts
 * pending import records. Also handles queue fan-out of per-file jobs
 * and the legacy single-pass course processing path.
 */

import sql from "../../database/pgsql";
import { getCanvasQueueAttemptLimit } from "../queue.ts";
...
import { syncAssignmentMetadata } from "./sync-assignments";
...
type AttachmentHandler = (
  ...
  assignmentFolderId: string | null,
) => Promise<void>;
...
async function lockUserTree(tx: import("postgres").TransactionSql, userId: string) {
  await tx`
...
export function parseJobCourses(job: { course_ids: unknown }) {
  const raw =
...
async function insertPendingFile(
  ...
  if (!resolvedMimeType || !PROCESSABLE_TYPES.has(resolvedMimeType)) return;
    ...
    modules.map((module) => async () => {
      ...
      const fileItems = items.filter(
        (item): item is CanvasModuleItem & { content_id: NonNullable<CanvasModuleItem["content_id"]> } =>
          item.type === "File" && item.content_id != null,
      );
...
async function processModules(
  ...
  const { client, jobId, storage } = ctx;
  if (!storage) throw new Error("Canvas file processing requires storage");
  const { data: modules } = await fetchResource(
    (id) => client.getModules(id),
    courseId,
    userId,
  ...
  await pooled(
    modules.map((module) => async () => {
      ...
      const resolved = metaResults.flatMap((result) =>
...
export async function processCourse(courseInput: unknown, userId: string, ctx: ImportContext) {
  const course = normalizeCanvasCourseSelection(courseInput);
  const courseId = String(course.id);
  const { title: courseTitle, academicYear } = cleanCourseName(
    course.course_code,
    course.name,
