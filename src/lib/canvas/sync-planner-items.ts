...
interface PlannerClient {
  getPlannerItems(startDate: string, endDate: string): Promise<{
...
interface SyncCanvasPlannerItemsParams {
  userId?: string;
  canvasDomain?: string;
...
async function upsertPlannerItem(userId: string, item: NormalizedPlannerItem) {
  await sql`
...
export async function syncCanvasPlannerItems(params: SyncCanvasPlannerItemsParams = {}) {
  const {
    userId,
    canvasDomain,
    canvasUserId = null,
