...
export function selectCards(
    ...
    if (dueCards.length === 0 && uncoveredChunkIds.length === 0 && masteredCards.length === 0) {
    ...
    const remaining = maxQuestions - dueCount;
    ...
    const retentionRemaining = maxQuestions - dueCount - newCount;
...
export async function getSessionCandidates(
    userId: string,
    chunkIds: string[],
): Promise<{ dueCards: any[]; uncoveredChunkIds: string[]; masteredCards: any[] }> {
