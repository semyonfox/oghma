...
export function buildGenerationPrompt(
    ...
    switch (questionType) {
        case 'mcq':
            typeInstruction = 'Generate a multiple choice question with exactly 4 options. Exactly one option must be correct.';
            break;
        case 'true_false':
            typeInstruction = 'Generate a true/false question. Set options to [{text: "True", is_correct: true/false}, {text: "False", is_correct: true/false}].';
            break;
        case 'fill_in':
            typeInstruction = 'Generate a fill-in-the-blank question. Set options to null. The correct_answer should be the expected word or short phrase.';
            break;
        default:
...
export async function generateQuestion(
    ...
    const prompt = buildGenerationPrompt(chunkText, moduleName, bloomLevel, questionType);

    let raw: string;
    try {
        raw = await callLLM(prompt);
    } catch (err) {
        logger.error('quiz generation LLM call failed', { error: err });
        return null;
    }

    const parsed = parseGeneratedQuestion(raw);
    if (!parsed) {
        logger.warn('quiz generation: failed to parse LLM response', { raw: raw.slice(0, 500) });
        return null;
    }
    ...
    try {
        ...
        logger.error('quiz generation: failed to save question', { error: err });
        return null;
    }
