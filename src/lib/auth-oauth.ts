...
const ALWAYS_VERIFIED_PROVIDERS = new Set(['google', 'apple']);
 ...
 * google and apple always verify. github and azure-ad require checking the profile.
 */
...
export async function syncProfileToLogin(
    userId: string,
    profile: { name?: string | null; image?: string | null; locale?: string | null },
): Promise<void> {
    await (sql as any)`
...
export async function findOrCreateOAuthUser(
    ...
    const existing = await findOAuthAccount(profile.provider, profile.providerAccountId);
    if (existing) {
    ...
    const emailVerified = isEmailVerifiedByProvider(profile.provider, providerProfile);
