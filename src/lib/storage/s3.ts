...
// original: libs/server/store/providers/s3.ts
// adapted for socsboard
...
export class StoreS3 extends StoreProvider {
  ...
  constructor(config: S3Config) {
    ...
    if (!config.accessKey || !config.secretKey) {
      this.logger.warn(
        'Environment variables STORAGE_ACCESS_KEY or STORAGE_SECRET_KEY is missing. Trying to use IAM role credentials instead ...'
      );
    }
    this.config = config;
  }

  // workaround for MinIO with port numbers in endpoint
  ...
  async copyObject(fromPath: string, toPath: string, options: ObjectOptions) {
    await this.client.send(
      new CopyObjectCommand({
        ...
        ContentEncoding: options?.headers?.contentEncoding,
        ContentType: options?.contentType,
        MetadataDirective: !options?.meta || Object.keys(options.meta).length === 0 ? 'COPY' : 'REPLACE',
      })
    );
  }
}
