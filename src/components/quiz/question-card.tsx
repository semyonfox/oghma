...
export default function QuestionCard({ question, moduleName, onAnswer }: QuestionCardProps) {
    ...
    const handleSubmit = () => {
        ...
        if (question.question_type === 'fill_in') {
            const correct = fillInAnswer.trim().toLowerCase() === question.correct_answer.trim().toLowerCase();
        ...
        <div className="flex-1 flex flex-col justify-center gap-5">
                        ...
                        return (
                            ...
                            >
                                <div className={`w-6 h-6 rounded-full border flex items-center justify-center text-xs flex-shrink-0 ${
                                    i === selected
                                        ? submitted
                                            ? option.is_correct ? 'bg-success-500 border-success-500 text-white' : 'bg-error-500 border-error-500 text-white'
                                ...
                                <span className={submitted && option.is_correct ? 'text-text' : i === selected && submitted && !option.is_correct ? 'text-text-tertiary line-through' : 'text-text-secondary'}>
                                    {option.text}
                                </span>
                            </button>
                        );
            ...
            {question.question_type === 'fill_in' && (
                <div className="flex gap-2">
                    <input
                        ...
                        onKeyDown={(e) => e.key === 'Enter' && !submitted && fillInAnswer.trim() && handleSubmit()}
                    />
                </div>
            )}
            ...
            {!submitted && (
                <button
                    ...
                    disabled={(question.options && selected === null) || (question.question_type === 'fill_in' && !fillInAnswer.trim())}
