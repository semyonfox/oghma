...
export function localizeText(node: ReactNode, t: Translate): ReactNode {
  ...
  return React.cloneElement(node as React.ReactElement<{ children?: ReactNode }>, {
    children: localizeText(props.children, t),
  });
}
...
export async function InfoSection({ title, children }: { title: string; children: ReactNode }) {
  const { t } = await getServerI18n();
  return (
    <section>
