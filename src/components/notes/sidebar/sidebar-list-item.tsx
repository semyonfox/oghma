...
const SidebarListItem: FC<{
    ...
    const { data: ingestionData } = useSWR(
        !isFolder ? `/api/ingestion-status?noteId=${item.id}` : null,
    ...
    const isIndexing =
        ingestionData?.status === "pending" || ingestionData?.status === "processing";
            ...
            >
              ...
              {(emoji
                  ...
                  M
                            ...
                            {isIndexing && (
                                <span className="ml-1 text-blue-400 text-xs" title="Indexing for search...">
                  ⏳
                </span>
                            )}
                            {syncStatus === "new" && (
