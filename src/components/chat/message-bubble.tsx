...
const ThinkingBlock: FC<{
  ...
  return (
    <div className="border border-border-subtle rounded-lg overflow-hidden bg-surface/50">
      <button
        type="button"
        onClick={() => setExpanded(!expanded)}
        className="w-full flex items-center gap-2 px-3 py-2 text-left hover:bg-subtle/50 transition-colors"
      >
        {isStreaming ? (
      ...
      {expanded && (
        <div className="px-3 pb-3 pt-2 text-xs text-text-tertiary leading-relaxed border-t border-border-subtle italic whitespace-pre-wrap max-h-72 overflow-y-auto">
          {text}
        </div>
      )}
    </div>
