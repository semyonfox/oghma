...
const WELCOME_COMPACT = "Hi! Ask me anything about this note.";
const WELCOME_FULL = "Hi! I'm your AI study assistant. Ask me anything about your notes, or upload a PDF to get started.";
        ...
        <div className="flex flex-wrap gap-1 mt-2">
            {sources.map((s) => (
                ...
                >
                    ...
                    <span className="max-w-[120px] truncate">{s.title || 'Untitled'}</span>
                </a>
            ))}
        </div>
    );
...
const ChatInterface: FC<ChatInterfaceProps> = ({
    ...
    const send = async () => {
        ...
        const userMsg: Message = { id: makeId(), role: 'user', content: text, timestamp: Date.now() };
        ...
        <div className={`flex flex-col h-full ${className}`}>
            ...
            {(noteId || noteTitle) && (
                <div className="flex-shrink-0 flex items-center gap-2 px-4 py-2 border-b border-white/8 bg-white/3 text-xs text-gray-400">
                    ...
                    <span>Context: </span>
                    <a href={`/notes/${noteId}`} className="text-indigo-300 hover:text-indigo-200 truncate transition-colors">
                        ...
                        <div className={`max-w-2xl ${m.role === 'user' ? 'max-w-lg' : ''}`}>
                                    ...
                                    <ReactMarkdown
                                        ...
                                        components={{
                                            ...
                                            code: ({ children, className: cls }) => {
                                                ...
                                                return isBlock
                                                    ? <code className="block bg-black/30 rounded-lg p-3 text-xs font-mono my-2 overflow-x-auto whitespace-pre">{children}</code>
                                            ...
                                            h4: ({ children }) => <h4 className="font-medium text-white mt-2 mb-1">{children}</h4>,
                                        }}
                                    >
                                        {m.content}
                                    </ReactMarkdown>
                            ...
                            {m.sources && m.sources.length > 0 && (
                                <div className="mt-2 ml-1">
                                    <p className="text-[11px] text-gray-600 mb-1">Sources</p>
                                    <SourceChips sources={m.sources} />
                                </div>
                            )}
            ...
            <div className="flex-shrink-0 border-t border-white/8 bg-gray-900 px-4 md:px-8 lg:px-12 py-4">
                <div className="max-w-3xl mx-auto">
                    ...
                    <p className="text-center text-[11px] text-gray-700 mt-2">
                        AI answers are based on your notes. Always verify important information.
                    </p>
                </div>
            </div>
        </div>
