  ...
  return (
    <div className="grid grid-cols-1 gap-y-8 sm:max-w-xl">
      ...
      <div className="rounded-md bg-white/5 p-4 ring-1 ring-white/10">
        <h3 className="text-sm font-semibold text-white mb-2">How to generate your Canvas API token</h3>
        <ol className="list-decimal list-inside space-y-1 text-sm text-gray-400">
          <li>Log into your Canvas account</li>
          <li>Click your profile picture → <span className="text-gray-300">Settings</span></li>
          <li>Scroll down to <span className="text-gray-300">Approved Integrations</span></li>
          <li>Click <span className="text-gray-300">+ New Access Token</span></li>
          <li>Give it a name (e.g. "OghmaNotes") and click <span className="text-gray-300">Generate Token</span></li>
          <li>Copy the token and paste it below — Canvas will only show it once</li>
        </ol>
      </div>
        ...
        <>
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-gray-400">
              Your institution's Canvas URL e.g. <span className="text-gray-300">dcu.instructure.com</span>
            </p>
            <div className="mt-2">
              <input
                id="canvas-domain"
        ...
        <>
                ...
                <label key={course.id} className="flex items-start gap-3 cursor-pointer">
                  ...
                  <div>
                    ...
                    {course.modules?.length > 0 && (
                      <p className="text-xs text-gray-500">{course.modules.length} module{course.modules.length !== 1 ? 's' : ''}</p>
                    )}
                  </div>
                </label>
              ))}
          ...
          {importSummary && (
            <div className="rounded-md bg-white/5 p-4 ring-1 ring-white/10 text-sm text-gray-300 space-y-1">
              ...
              {importSummary.forbidden > 0 && (
                <p>🔒 Restricted by lecturer: {importSummary.forbidden} — see below to upload manually</p>
              )}
              {importSummary.failed > 0 && <p>❌ Failed: {importSummary.failed}</p>}
              {importSummary.skipped > 0 && <p>⏭ Skipped (unsupported type): {importSummary.skipped}</p>}
            </div>
          )}
          ...
          <div className="flex gap-3">
            <button
              ...
              className="rounded-md bg-indigo-500 px-3 py-2 text-sm font-semibold text-white hover:bg-indigo-400 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isImporting ? 'Importing...' : `Import ${selectedCourseIds.length > 0 ? `(${selectedCourseIds.length})` : ''}`}
            </button>
            <button
              type="button"
