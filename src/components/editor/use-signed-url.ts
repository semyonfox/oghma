...
export function useSignedUrl(sourcePath?: string, fileId?: string) {
    ...
    const loadUrl = async () => {
      ...
      if (!resolvedPath && fileId) {
        try {
          const noteRes = await fetch(
            `/api/notes/${fileId}?fields=s3Key,content`,
          );
          if (!noteRes.ok) {
            if (!cancelled) {
              setError("note-fetch-failed");
              setLoading(false);
            }
            return;
          }
      ...
      try {
        const response = await fetch(
          `/api/upload?path=${encodeURIComponent(resolvedPath)}`,
        );
        if (!response.ok) {
          if (!cancelled) {
            setUrl("");
