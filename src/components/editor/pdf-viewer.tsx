...
const PDFViewer: FC<PDFViewerProps> = ({ file, pane }) => {
  ...
  const { url: pdfPath, loading, error: urlError } = useFileUrl(file.sourcePath, file.fileId);
      ...
      >
        {loading ? (
          <div className="flex flex-col items-center gap-3 mt-16">
            ...
            <span className="text-sm text-text-tertiary">{t('pdf_viewer.loading')}</span>
          </div>
        ) : !pdfPath ? (
          <div className="flex flex-col items-center gap-3 mt-16 text-center px-6">
            <svg className="w-8 h-8 text-red-400/60" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
          ...
          <Document
            ...
            loading={
              <div className="flex flex-col items-center gap-3 mt-16">
                ...
                <span className="text-sm text-text-tertiary">{t('pdf_viewer.preparing')}</span>
              </div>
            }
            error={<div className="text-red-500 mt-16">{t('pdf_viewer.error')}</div>}
          >
