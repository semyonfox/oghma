...
interface DayCell {
  ...
  isToday: boolean;
  isSelected: boolean;
  assignments: { id: string; title: string; courseColor: string | null; status: string }[];
  timeBlocks: { id: string; title: string | null; courseColor: string | null }[];
}
...
export default function MonthView() {
  const { currentDate, selectedDate, setSelectedDate, timeBlocks, fetchTimeBlocks } = useCalendarStore();
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="grid grid-cols-7 gap-px border-b border-border-subtle bg-white/5 text-center text-xs font-medium text-text-tertiary">
        {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((d) => (
          <div key={d} className="bg-surface py-2">{d}</div>
        ))}
      </div>
          ...
          {days.map((day) => (
            ...
            >
              ...
              <div className="mt-1 space-y-0.5">
                ...
                {day.timeBlocks.slice(0, Math.max(0, 2 - day.assignments.length)).map((tb) => (
                  <div
                    key={tb.id}
