...
function originForInitialPage(): "direct" | "internal" | "external" {
  if (!document.referrer) return "direct";
  try { return new URL(document.referrer).origin === window.location.origin ? "internal" : "external"; } catch { return "direct"; }
}

type NavigationContext = { placement?: string; action?: string };
function navigationContext(element: HTMLElement): NavigationContext {
...
function targetPath(element: Element): string | null {
...
function inferClickEvent(element: HTMLElement): string | null {
...
function clickProperties(element: HTMLElement): Record<string, string | undefined> {
  return {
...
export default function MarketingTracker() {
  ...
  const sentClickTransition = useRef<{ fromPath: string; toPath: string } | null>(null);
  const pathChain = useRef<string[]>([]);
  const lastCta = useRef<{ path: string; placement?: string; action?: string } | null>(null);
    ...
    function onClick(event: MouseEvent) {
      ...
      if (eventName) {
        trackMarketingEvent(eventName, {
          ...
          targetUrl: element instanceof HTMLAnchorElement
            ? element.href
            : element instanceof HTMLButtonElement
              ? element.formAction || null
              : null,
          ...context,
