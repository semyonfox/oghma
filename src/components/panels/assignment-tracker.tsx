...
import useAssignmentStore, { type Assignment, type AssignmentTab } from '@/lib/notes/state/assignments.zustand';
import usePomodoroStore from '@/lib/notes/state/pomodoro.zustand';
import useI18n from '@/lib/notes/hooks/use-i18n';
import NewTaskModal from './new-task-modal';

function CompletionRing({ done, total, size = 72 }: { done: number; total: number; size?: number }) {
  const center = size / 2;
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      ...
      <circle cx={center} cy={center} r={outerR} fill="none" stroke="currentColor" strokeWidth={5} className="text-white/10" />
      <circle
      ...
      <circle cx={center} cy={center} r={innerR} fill="none" stroke="currentColor" strokeWidth={4} className="text-white/10" />
      <circle
      ...
      />
      {/* center text */}
      <text x={center} y={center - 2} textAnchor="middle" fill="white" fontSize={14} fontWeight="bold">
        {total > 0 ? `${Math.round(pct * 100)}%` : '—'}
      </text>
      <text x={center} y={center + 12} textAnchor="middle" fill="#888" fontSize={9}>
        {done}/{total}
      </text>
    </svg>
  );
...
export default function AssignmentTracker() {
  const { t } = useI18n();
  const {
    assignments, loading, courseFilter, activeTab,
    fetchAssignments, syncFromCanvas, setCourseFilter, setActiveTab,
  } = useAssignmentStore();
  ...
  const courses = useMemo(() => {
    const names = new Set(assignments.map((a) => a.course_name).filter(Boolean));
  ...
  return (
    <div className="h-full flex flex-col">
        ...
        <Listbox value={courseFilter} onChange={setCourseFilter}>
          <div className="relative flex-1">
            ...
            <ListboxOptions className="absolute z-10 mt-1 max-h-40 w-full overflow-auto rounded border border-border-subtle bg-surface py-1 text-xs shadow-lg">
              <ListboxOption value={null} className="cursor-pointer px-2.5 py-1.5 text-text-secondary hover:bg-white/5 data-[focus]:bg-white/5">
                {t('All Courses')}
              </ListboxOption>
              {courses.map((c) => (
                <ListboxOption key={c} value={c} className="cursor-pointer px-2.5 py-1.5 text-text-secondary hover:bg-white/5 data-[focus]:bg-white/5">
                  {c}
                </ListboxOption>
              ))}
            </ListboxOptions>
      ...
      <div className="flex gap-1 px-3 pb-2">
        ...
        <button onClick={() => setActiveTab('late')} className={tabClasses('late')}>
          {t('Late')}
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-3 space-y-2">
        {loading ? (
          <p className="text-xs text-text-tertiary py-4 text-center">{t('Loading...')}</p>
        ) : filtered.length === 0 ? (
          <p className="text-xs text-text-tertiary py-4 text-center">
            {activeTab === 'done' ? t('No completed assignments') : t('No assignments')}
          </p>
        ) : (
          filtered.map((a) => (
            <div
            ...
            >
              ...
              <p className="mt-1 text-xs font-medium text-text-secondary leading-tight">{a.title}</p>
              <div className="mt-1.5 flex items-center gap-2 text-[10px] text-text-tertiary">
                ...
                {a.points_possible != null && (
                  <span>{a.score != null ? `${a.score}/` : ''}{a.points_possible} pts</span>
                )}
              </div>
              {/* allotment progress */}
                ...
                <div className="mt-2">
                  <div className="h-1 rounded-full bg-white/10">
                    <div
                      ...
                      style={{ width: `${Math.min(100, (a.logged_hours / a.estimated_hours) * 100)}%` }}
                    />
                  </div>
